...
public class TesteProdutorFilaAcknowledge {
...
//        for(int i = 0; i < 1000 ; i++){
            Message message = session.createTextMessage("<pedido><id>123</id><nome>Acknowledge</nome></pedido>");
            messageProducer.send(message); // Esse cara faz o envio da mensagem para a fila.
//        }

        System.out.println("[TesteProdutorFilaAcknowledge] Teste Antes do nextLine()");

        // Deixa a aplicação "pausada" enquanto algo não é digitado no teclado:
        new Scanner(System.in).nextLine();

        System.out.println("[TesteProdutorFilaAcknowledge] Teste Depois do nextLine()");
